...
const NetTest = ({ idArray }) => {
  ...
  function generateShapes() {
    ...
    for ( let i = 0 ; i < idArray.length; i++ ) {
      if (idArray.includes(String(ssnArray[i].id))) {
        ...
        shapes.push({
          id: ssnArray[i].id,
          x: Math.random() * (width_g-20),
          y: Math.random()* (height_g-10),
  ...
  return (
    <Stage width={width_g} height={height_g} fill={'white'}>
      <Layer>
      	{connectors.map(con => {
